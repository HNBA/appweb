...
DOCUMENTATION = '''
---
See https://github.com/ansible/ansible-modules-core/pull/3579 line 793
module: yum_phenix
...
- name: install the latest version of Apache
  yum_phenix: name=httpd state=latest
- name: remove the Apache package
  yum_phenix: name=httpd state=absent
- name: install the latest version of Apache from the testing repo
  yum_phenix: name=httpd enablerepo=testing state=present
- name: install one specific version of Apache
  yum_phenix: name=httpd-2.2.29-1.4.amzn1 state=present
- name: upgrade all packages
  yum_phenix: name=* state=latest
- name: install the nginx rpm from a remote repo
  yum_phenix: name=http://nginx.org/packages/centos/6/noarch/RPMS/nginx-release-centos-6-0.el6.ngx.noarch.rpm state=present
- name: install nginx rpm from a local file
  yum_phenix: name=/usr/local/src/nginx-release-centos-6-0.el6.ngx.noarch.rpm state=present
- name: install the 'Development tools' package group
  yum_phenix: name="@Development tools" state=present
- name: install the 'Gnome desktop' environment group
  yum_phenix: name="@^gnome-desktop-environment" state=present
'''
...
def is_installed(module, repoq, pkgspec, conf_file, qf=def_qf, en_repos=None, dis_repos=None, is_pkg=False):
    ...
    if not repoq:
        ...
        try:
            ...
            module.fail_json(msg="Failure talking to yum_phenix: %s" % e)
